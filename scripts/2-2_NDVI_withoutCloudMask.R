#######################
####### Data NDVI without cloud mask - R script - R script
...
if (!file.exists(paste0(dataPath,'/NDVI_MODIS')))
  dir.create(paste0(dataPath,'/NDVI_MODIS'))
...
for (i in 1:length(dlist)) {                   # start of the outer for-loop
  ...
  for (k in 1:nlayers(ndvi)) {     # start of the inner for-loop
    ...
    plot(ndvi[[k]],
         ...
         main=paste("NDVI"," (no cloud mask)"," sample ",doy," ",year,sep="")) # automizes the title of the plot.
    ...
    writeRaster(ndvi[[k]], filename=paste(fold_tif,"/",doy,"_",year,".tif",sep=""), format="GTiff", atatype='INT1S', overwrite=TRUE)
